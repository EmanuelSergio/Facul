...
public class Algoritmos2 {
    ...
    // Conta quantas vezes um determinado elemento aparece no vetor com do-while
    // Conta quantas vezes um valor específico aparece em um vetor de inteiros usando a estrutura do-while.
    public static int countOccurrencesDoWhile(int[] arr, int key) {
        int count = 0;
        int i = 0;
        do {
            if (arr[i] == key) {
    ...
    // Verifica se o vetor é um palíndromo com do-while
    // Verifica se os elementos de um vetor de inteiros formam um palíndromo (ou seja, são iguais quando lidos de trás para frente) usando a estrutura do-while.
    public static boolean isPalindromeDoWhile(int[] arr) {
        int left = 0, right = arr.length - 1;
        do {
            if (arr[left] != arr[right]) {
                return false;
    ...
    // Inverte a ordem dos elementos no vetor com while
    // Inverte os elementos de um vetor de inteiros em ordem reversa usando a estrutura while.
